...
export default function Search() {
    ...
    //https://api.hgbrasil.com/weather?key=705d265d&city_name=Campinas,SP   

    const response = await api.get(`/weather?key=${key}&city_name=${input} `);
    // console.log(response.data);

    if(response.data.by === 'default'){
        setError('Oops, cidade não encontrada!');
        setInput('');
        setCity(null);
        Keyboard.dismiss();
        return;
    ...
    return(
        <SafeAreaView style={styles.container}> 
            ...
            {error && <Text style={{marginTop: 85, fontSize: 18 }}>{error}</Text>}
